...
import { NBA_CATALOG_ENGINE, NBA_POSITION_GROUP_OPTIONS } from "../src/lib/nba-catalog-server";
...
import { NBA_DEFAULT_ROOM_SETTINGS as DEFAULT_ROOM_SETTINGS } from "../src/lib/nba-settings-server";
import type {
  LeaveIntent,
  PlayerCatalogEntry,
  RoomClosedReason,
...
const { CATALOG, CATALOG_YEAR_RANGE, buildBalancedPlayerDeck, findPlayerById, getEligiblePlayers } = NBA_CATALOG_ENGINE;
...
export default class RoomParty implements Party.Server {
  ...
  private async handleUpdateSettings(participantId: string, next: Partial<RoomSettings>): Promise<RoomSnapshot> {
    ...
    if (!NBA_POSITION_GROUP_OPTIONS.includes(merged.positionGroup)) {
